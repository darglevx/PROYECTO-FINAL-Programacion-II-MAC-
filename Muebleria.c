...
// Opción 5: Crear gráfica de columnas
...
void graficaColumnas()
{
    ...
    int xMax, yMax, xInicio, anchoBarra;
    float escala, barraMax = 0, yBase = 0;
    char respuesta;
    ...
    if(archivo == NULL)
    {
        printf("\n\tNo hay productos registrados para graficar\n");
        pausa();
        return;
    }
    ...
    // Cargamos todos los registros a la lista en la memoria y cerramos el archivo
    fread(lista, sizeof(Producto), n, archivo);
    fclose(archivo);

    // 2. CÁLCULO DE VALOR MÁXIMO
    float max_unidades = 0;

    // Necesitamos saber cuál es el producto con más unidades vendidas para que sea nuestra barra más alta
    for(i = 0; i < n; i++)
    {
        if(!lista[i].borrado) // Solo tomamos en cuenta los productos activos
        {
            activos++;

            if(lista[i].unidades_vendidas > max_unidades)
            {
                max_unidades = lista[i].unidades_vendidas;
            }
        }
    }

    // Si no hay productos activos no podemos graficar
    if(activos == 0)
    {
        printf("\n\tNo hay productos activos para graficar\n");
        free(lista);
        pausa();
        return;
    }
    ...
    printf("\n\t¿Deseas dar las dimensiones de la pantalla (s/n)?\n");

    scanf(" %c", &respuesta);
    Limpiar_buffer(); // Limpia el salto de línea

    // Para conertir la respuesta a minúscula sin importar si se tecleó ´S´ o ´s´
    ...
    {
        // Se toman las dimensiones máximas de la pantalla
        xMax = getmaxwidth();
        yMax = getmaxheight();
    }

    // 4. INICIO DE MODO GRAFICO
    initwindow(xMax, yMax, "Gráfica-Unidades Vendidas");
    setbkcolor(WHITE);
    cleardevice();

    // 5. CÁCULO DE PROPORCIONES
    /* (activos * 2-1) hace que el espacio entre las barras sea del mismo tamaño que la barra*/
    anchoBarra = (xMax * 0.20) / (activos * 2-1);

    xInicio = (xMax - ((activos * 2 - 1) * anchoBarra)) / 2;

    escala = 230.0 / max_unidades;

    // Base donde se apoyan las barras
    yBase = yMax * 0.75;
    ...
    outtextxy((xMax / 2) - (textwidth("Unidades Vendidas") / 2), yMax * 0.1, "Unidades Vendidas");

    // Definimos variables para graficar
    int altura, colorActual = 1, txtAncho, puntos[10]; // Arreglo para guardar las coordenadas (x,y) de las esquinas
    char texto[10];
    ...
    for(i=0; i<n; i++)
    {
        if(!lista[i].borrado)
        {
            colorActual = (colorActual == 15) ? 1 : colorActual;

            // Altura de la barra actual
            altura = lista[i].unidades_vendidas * escala;

            // Dibujamos la barra
            setcolor(colorActual);
            setfillstyle(SOLID_FILL, colorActual);

            // Definimos las coordenadas (x,y) de la barra
            puntos[0] = xInicio;                  // Esquina inferior izquierda (X)
            puntos[1] = yBase;                    // Esquina inferior izquierda (Y)

            puntos[2] = xInicio;                  // Esquina superior izquierda (X)
            puntos[3] = yBase - altura;           // Esquina superior izquierda (Y)

            puntos[4] = xInicio + anchoBarra;     // Esquina superior derecha (X)
            puntos[5] = yBase - altura;           // Esquina superior derecha (Y)

            puntos[6] = xInicio + anchoBarra;     // Esquina inferior derecha (X)
            puntos[7] = yBase;                    // Esquina inferior derecha (Y)

            puntos[8] = xInicio;                  // Volvemos a la inferior izquierda para cerrar
            puntos[9] = yBase;
            ...
            // Texto centrado justo debajo de su respectiva barra
            txtAncho = textwidth(lista[i].clave);
            outtextxy(xInicio + (anchoBarra / 2) - (txtAncho / 2), yBase + 20, lista[i].clave);

            // Texto con la cantidad de unidades vendidas
            setcolor(DARKGRAY);
            settextstyle(SANS_SERIF_FONT, HORIZ_DIR, 1);
            sprintf(texto, "%.0f", lista[i].unidades_vendidas);

            // Centrar el número
            txtAncho = textwidth(texto);
            outtextxy(xInicio + (anchoBarra / 2) - (txtAncho / 2), yBase - altura - 30, texto);

            // Para continuar con el ciclo
            xInicio = xInicio + anchoBarra * 2;
            colorActual++;
        }
    }
